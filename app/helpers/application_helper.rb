...
module ApplicationHelper
  ...
  def show_map(address, html)
    link_to(raw("<span>View map</span>"), "/gmaps?address=#{address}&html=#{html}&placeValuesBeforeTB_=savedValues&TB_iframe=true&height=410&width=505&inlineId=hiddenDiv", :class=>"thickbox", :title => "View map")  
  end
  ...
  def show_attach(wall)
    ...
    if wall.user_wall_link
      ...
      if wall.user_wall_link.image_link != ""
        img_link = link_to(raw(image_tag("/gadgets_proxy/link?url=#{raw(wall.user_wall_link.image_link)}", :style => "width:92px;height:68px")), link, :target => "_blank")
        image = get_image_wall(wall.id, img_link)
      end
