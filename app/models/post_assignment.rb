...
class PostAssignment < ActiveRecord::Base
  ...
  scope :interesting, :conditions => ["(Select Count(*) From favorites Where favorites.favorable_id = post_assignments.post_id And favorable_type = ?) > ?", "Post", 10], :order => "id DESC"
...
#    return posts
  ...
  def self.recent_interesting(school_id)
    ...
    objs = Post.find_by_sql("select pa.id,p.* from posts as p right join (select * from post_assignments) as pa on p.id = pa.post_id
inner join
