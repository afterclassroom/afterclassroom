...
class PostLectureNote < ActiveRecord::Base
  # Validations
  ...
  scope :due_date, :conditions => ["post_lecture_notes.due_date > ?", Time.now], :order => "due_date ASC"
  scope :interesting, :conditions => ["(Select Count(*) From favorites Where favorites.favorable_id = s.post_id And favorable_type = ?) > ?", "Post", 10], :order => "id DESC"
  scope :random, lambda { |random| {:order => "RAND()", :limit => random }}
  scope :previous, lambda { |att| {:conditions => ["post_lecture_notes.id < ?", att], :order => "id ASC"} }
  scope :nexts, lambda { |att| {:conditions => ["post_lecture_notes.id > ?", att], :order => "id ASC"} }
  ...
  def self.paginated_post_conditions_with_option(params, school)
    ...
    post_as = PostLectureNote.ez_find(:all, :include => [:post], :order => "posts.created_at DESC") do |post_lecture_note, post|
...
#    return posts
  ...
  def self.recent_interesting(school_id)
    objs = Post.recent_interesting(school_id,"post_lecture_notes")
  end
end
