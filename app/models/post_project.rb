...
class PostProject < ActiveRecord::Base
  ...
  named_scope :due_date, :conditions => ["post_projects.due_date > ?", Time.now], :order => "due_date DESC"
  named_scope :interesting, :conditions => ["(Select Count(*) From favorites Where favorites.favorable_id = post_projects.post_id And favorable_type = ?) > ?", "Post", 10]
