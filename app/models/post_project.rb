...
class PostProject < ActiveRecord::Base
  ...
  scope :due_date, :conditions => ["post_projects.due_date > ?", Time.now], :order => "due_date DESC"
  ...
  scope :previous, lambda { |att| {:conditions => ["post_projects.id < ?", att]} }
  scope :next, lambda { |att| {:conditions => ["post_projects.id > ?", att]} }
