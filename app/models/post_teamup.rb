...
class PostTeamup < ActiveRecord::Base
  ...
  scope :with_status, lambda { |st| {:conditions => ["post_teamups.rating_status = ?", st], :order => "created_at DESC"} }
  scope :with_category, lambda { |c| {:conditions => ["post_teamups.teamup_category_id = ?", c], :order => "created_at DESC"} }
  ...
  def self.related_posts(school, category_id)
    posts = []
    post_as = if school 
      self.random(5).with_school(school).with_category(category_id)
    else
      self.random(5).with_category(category_id)
    end
    post_as.select {|p| posts << p.post}
    posts
  end
