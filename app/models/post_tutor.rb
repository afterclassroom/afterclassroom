...
class PostTutor < ActiveRecord::Base
  ...
  scope :with_type, lambda { |tp| {:conditions => ["post_tutors.tutor_type_id = ?", tp], :order => "id DESC"} }
  scope :with_status, lambda { |st| {:conditions => ["post_tutors.rating_status = ?", st], :order => "id DESC"} }
