...
class PostTutor < ActiveRecord::Base
  ...
  scope :with_type, lambda { |tp| {:conditions => ["post_tutors.tutor_type_id = ?", tp], :order => "created_at DESC"} }
  scope :with_status, lambda { |st| {:conditions => ["post_tutors.rating_status = ?", st], :order => "created_at DESC"} }
  ...
  scope :nexts, lambda { |att| {:conditions => ["post_tutors.id > ?", att], :order => "id ASC"} }
