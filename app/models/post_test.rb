...
class PostTest < ActiveRecord::Base
  ...
  scope :interesting, :conditions => ["(Select Count(*) From favorites Where favorites.favorable_id = post_tests.post_id And favorable_type = ?) > ?", "Post", 10], :order => "posts.created_at DESC"
