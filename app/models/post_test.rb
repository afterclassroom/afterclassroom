...
class PostTest < ActiveRecord::Base
  ...
  scope :interesting, :conditions => ["(Select Count(*) From favorites Where favorites.favorable_id = post_tests.post_id And favorable_type = ?) > ?", "Post", 10]
  scope :random, lambda { |random| {:order => "RAND()", :limit => random }}
  scope :previous, lambda { |att| {:conditions => ["post_tests.id < ?", att]} }
  scope :next, lambda { |att| {:conditions => ["post_tests.id > ?", att]} }
