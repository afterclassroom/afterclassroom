...
class PostExam < ActiveRecord::Base
  ...
  scope :interesting, :conditions => ["(Select Count(*) As fr From favorites Where favorites.favorable_id = post_exams.post_id And favorable_type = ?) > ?", "Post", 0], :order => "id DESC"
...
#    return posts
  ...
  def self.recent_interesting(school_id)
    objs = Post.recent_interesting(school_id,"post_exams")
  end
