...
class PostJob < ActiveRecord::Base
  ...
  scope :with_type, lambda { |tp| {:conditions => ["post_jobs.job_type_id = ?", tp], :order => "created_at DESC"} }
  scope :without_type, lambda { |tp| {:conditions => ["post_jobs.job_type_id <> ?", tp], :order => "created_at DESC"} }
  scope :with_status, lambda { |st| {:conditions => ["post_jobs.rating_status = ?", st], :order => "created_at DESC"} }
