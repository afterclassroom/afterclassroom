class CacheJob < Struct.new(:id, :class_name, :school_id, :params)
  ...
  def perform
    ...
    case class_name
      ...
      when "PostTutor"
        post = eval(class_name).find(id).post
        Rails.cache.write("index_#{class_name}_type#{post.tutor_type_id}_#{school_id}_year_department_over", PostTutor.paginated_post_conditions_with_option(params, school_id, post.tutor_type_id))
        list.each do |over|
          params[:department] = post.department_id
          params[:year] = post.school_year
          params[:over] = over
      ...
      when "PostBook"
        post = eval(class_name).find(id).post
        Rails.cache.write("index_#{class_name}_type#{post.book_type_id}_#{school_id}_year_department_over", PostBook.paginated_post_conditions_with_option(params, school_id, post.book_type_id))
        list.each do |over|
          params[:department] = post.department_id
          params[:year] = post.school_year
          params[:over] = over
      ...
      when "PostJob"
        post = eval(class_name).find(id).post
        Rails.cache.write("index_#{class_name}_type#{post.job_type_id}_#{school_id}_year_department_over", PostJob.paginated_post_conditions_with_option(params, school_id, post.job_type_id))
        list.each do |over|
          params[:department] = post.department_id
          params[:year] = post.school_year
          params[:over] = over
      ...
      when "PostAwareness"
        post = eval(class_name).find(id).post
        Rails.cache.write("index_#{class_name}_type#{post.awareness_type_id}_#{school_id}_year_department_over", PostAwareness.paginated_post_conditions_with_option(params, school_id, post.awareness_type_id))
        list.each do |over|
          params[:department] = post.department_id
          params[:year] = post.school_year
          params[:over] = over
      ...
      when "PostTeamup"
        post = eval(class_name).find(id).post
        Rails.cache.write("index_#{class_name}_category#{post.teamup_category_id}_#{school_id}_year_department_over", PostTeamup.paginated_post_conditions_with_option(params, school_id, post.teamup_category_id))
        list.each do |over|
          params[:department] = post.department_id
          params[:year] = post.school_year
          params[:over] = over
          Rails.cache.write("index_#{class_name}_category#{post.teamup_category_id}_#{school_id}_year#{post.school_year}_department#{post.department_id}_over#{over}", PostTeamup.paginated_post_conditions_with_option(params, school_id, post.teamup_category_id))
        end
      when "PostExamSchedule"
        post = eval(class_name).find(id).post
        Rails.cache.write("index_#{class_name}_type#{post.type_name}_#{school_id}_year_department_over", PostExamSchedule.paginated_post_conditions_with_option(params, school_id, post.type_name))
        list.each do |over|
          params[:department] = post.department_id
          params[:year] = post.school_year
          params[:over] = over
          Rails.cache.write("index_#{class_name}_type#{post.type_name}_#{school_id}_year#{post.school_year}_department#{post.department_id}_over#{over}", PostExamSchedule.paginated_post_conditions_with_option(params, school_id, post.type_name))
        end
    end
  end
end