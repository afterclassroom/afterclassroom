      ...
      @tokenizer.extract(p_data) do |b_data|
        begin
          ...
          if t_data
            ...
            when :async_invoke 
              async_method_invoke(t_data)
              # get status/result
            when :get_result 
              get_result_object(t_data)
              # sync method invocation
            when :sync_invoke
              method_invoke(t_data)
            when :start_worker
              start_worker_request(t_data)
            when :delete_worker
              delete_drb_worker(t_data)
            when :worker_info
              pass_worker_info(t_data)
            when :all_worker_info
              all_worker_info(t_data)
